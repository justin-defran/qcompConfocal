...
xMin=0.0 #only scan this region in the gap otherwise this should be 0 to 100
xMax=100.0
xStep=0.2
...
x_list=numpy.arange(x0,x0+1.0,0.1) #from x0 to x0+difference (final x) in steps of 0.1mm = 100um
y_list=numpy.arange(y0,y0+1.0,0.1)
z_list = numpy.arange(45.1,40.0, 0.2) #z is piezo stage z measured in um.
    ...
    for xp in x_list:
        ...
        for yp in y_list:
         #   Y.goto(yp)
            ...
            cb = plt.colorbar()
            plt.savefig("D:\workspace\Data\confocal\\2019-05-02\\x" + str(pos[0]) + " y" + str(pos[1]) + " z" + str(
                pos[2]) + ".png")
            np.savetxt("D:\workspace\Data\confocal\\2019-05-02\\x" + str(pos[0]) + " y" + str(pos[1]) + " z" + str(
