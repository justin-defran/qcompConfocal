...
z=25.5
freq=1000
z_list= np.arange(25.5,37.9,0.2)
...
for z in z_list:
    ...
    plt.savetxt("D:\workspace\Data\confocal\\2019-05-15\\" + str(z) + "counts.txt",Counts)


    im=plt.imshow(Counts,origin='lower',extent=[min(x), max(x), min(y), max(y)])
    cb=plt.colorbar()
    #plt.show()
    plt.savefig("D:\workspace\Data\confocal\\2019-05-15\\"+str(z)+".png")
    plt.close()
    print z
